...
**21e Contact intelligence (0.5)** — today Prospect / Customer / VIP are manual labels and the assistant only knows "new vs returning". Add: automatic promotion Prospect → Customer on a completed booking, paid payment, resolved job ticket or CRM status sync; VIP by tenant rule (lifetime value threshold, repeat-caller count, named accounts) or manual; contact context passed to the assistant at call start (name, status, VIP, last-call summary, open tickets/bookings) so it greets returning callers by name, never re-asks known details and follows tenant-defined VIP rules (e.g. always offer a transfer, priority ticket, dedicated department); status/VIP filters and lifetime value on Contacts and in the 21a revenue/CX measures.
