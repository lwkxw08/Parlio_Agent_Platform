...
20a–20d touch the call flow and were pulled forward ahead of Part F (done). 20f, 20g, 20i follow Part F as demand appears. **20e, 20h and 20j are deferred until the customer base justifies them** — not scheduled before Part F or Part H.
...
**21b Analytics UI (0.75) — DONE except export** — "Insights" section on Analytics (Demand, Trends, Resolution, Team, Customers, Revenue tabs) with premium chart primitives (`app/charts.tsx`: smooth shaded area line, donut, heat-map, funnel, columns, ranked bars, metric tiles) also applied to the existing Call volume / Calls by department cards; usage card now covers calls, minutes, transfers, tickets, bookings, SMS, WhatsApp, web chats and emails. **Still to do:** "Ask AI" wired to the new measures, CSV export, scheduled report emails.
...
**21e Contact intelligence (0.5)** — today Prospect / Customer / VIP are manual labels and the assistant only knows "new vs returning". Add: automatic promotion Prospect → Customer on a completed booking, paid payment, resolved job ticket or CRM status sync; VIP by tenant rule (lifetime value threshold, repeat-caller count, named accounts) or manual; users can always set or change Prospect / Customer / VIP / Blocked by hand on the contact (and from the call/ticket view), and a manual choice pins the status so automatic rules never override it (with an audit note of who changed it); contact context passed to the assistant at call start (name, status, VIP, last-call summary, open tickets/bookings) so it greets returning callers by name, never re-asks known details and follows tenant-defined VIP rules (e.g. always offer a transfer, priority ticket, dedicated department); status/VIP filters and lifetime value on Contacts and in the 21a revenue/CX measures.
