...
- **23c Small Studio/Calls exposures — done**: Studio → Speaking style has an **Interruptions** toggle (`turn.allow_interruptions`) and a **Glossary & pronunciations** list (term / say-as / meaning → prompt glossary + whole-word TTS substitution in `speakable`); Call detail has one-click **Block caller / Unblock caller** (`POST /v1/calls/{id}/block?block=`, tenant-admin only, publishes a new assistant version on change).
- **23d Knowledge sources — done**: Import FAQs accepts an uploaded PDF / .docx / .txt / .md / .csv (≤10 MB, first 200 PDF pages, encrypted or scanned PDFs rejected) and drafts suggestions through the existing review-and-apply flow; worker `search_website` tool (Studio → Business → **Live website search**) crawls up to 40 same-host pages of `business.website` (+ listed extra URLs on that host) once per process with a 15-minute cache, prewarmed at call start so the mid-call lookup is a local ranking; off-host links, redirects and binaries are never fetched; the prompt tells the assistant to say so rather than guess when nothing matches.
- **23e Call detail "Extracted details" panel — done**: captured fields and missing required fields from the existing `extracted` / `missed_fields` shown as a card under the summary.
...
- **23g No-card trial lifecycle — done**: no card at sign-up. Hourly sweep (`BillingService.sweep_trials`, `PARLIO_TRIAL_*` settings: 14-day trial, 3-day grace, close at 30 days): owner emails at T-7 / T-3 / T-1 and on expiry, then `past_due` grace (calls still answered), then `suspended` (worker plays a short "temporarily unavailable" notice and hangs up; dashboard shows a Subscribe wall and every non-Billing write returns 402), then `cancelled` at 30 days (numbers returned to the platform pool, data kept per retention). Each notice is sent once (`Subscription.trial_notices`). Stripe Checkout at any point leaves the lifecycle; staff **Extend trial** / **Convert** re-open it. `GET /v1/billing/trial` drives the dashboard banner (last 7 days) and wall.
...
### Phase 25 — Security assurance before paid launch (~1 session engineering + external test)
Not required for PoCs; required before taking paying customers and expected by dental/legal/enterprise buyers and Cyber Essentials Plus.
- **25a Automated (engineering, free)**: OWASP ZAP baseline + authenticated scan against app./api.parliotec.com (staging), dependency + container audits (already in CI: pip-audit, npm audit, image scan), a tenant-isolation sweep (every `/v1` object route exercised with two tenants' logins, RLS regression), auth hardening checks (rate limits on login/OTP, session expiry, CORS, headers). Fix findings; add the isolation sweep to CI.
- **25b External pen test (owner buys)**: CREST-accredited UK firm, grey-box web-app + API + multi-tenant isolation, retest included, shareable summary letter; ~£3–6k, 1–2 weeks. Repeat annually or after major changes. Candidates: Pentest People, Sencode, Evalian, Precursor, Bulletproof, Cyphere.
- **25c Certifications**: Cyber Essentials (self-assessed, ~£300–500) now; Cyber Essentials Plus after the pen test; publish both on the Trust centre with the pen-test summary.
