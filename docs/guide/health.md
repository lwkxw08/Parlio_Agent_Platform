...
The state of ParlioTec's platform components (telephony, voice, dashboard, messaging). A platform incident also shows a banner at the top of every page and on the public status page.
...
ParlioTec places one scripted test call a day to every business (a "book an appointment" style conversation) and after each deploy or configuration change. These appear here, not on your Calls page, and are not real customers. A failure raises an alert with the transcript. **Run test call now** triggers one on demand.

## SIP trunks and forwarding

For each connection on *Telephony*: registration state, last OPTIONS/INVITE result and audio quality (MOS, jitter, loss). **Diagnose** runs the full check and classifies the fault (your PBX, your provider, the network, or ParlioTec) with an evidence pack. **Diagnose forwarding** places a call to your forwarded number and reports whether the divert is working.
...
When a diagnosis points at your telephony provider, ParlioTec drafts a report you can **Copy** and send them, with timestamps, SIP call IDs and the evidence they will ask for.
