    ...
    <div className="grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
      <form className="section form" onSubmit={saveLimits}>
        ...
        <fieldset disabled={!canEdit} style={{ border: 0, padding: 0, margin: 0, display: "contents" }}>
          ...
          <label>Max team members / bookable calendars <input type="number" min={0} value={lim.max_resources ?? ""} onChange={(e) => setLim({ ...lim, max_resources: numOrNull(e.target.value) })} placeholder="plan default (0 = unlimited)" /></label>
