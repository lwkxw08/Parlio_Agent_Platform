...
export default async function Page({ searchParams }: { searchParams: Promise<{ tenant?: string; tab?: string }> }) {
  ...
  if (!tenant) return <><h1>Organisation settings</h1><p className="muted">No organisation yet — <Link href="/onboarding">set one up</Link>.</p></>;
    ...
    <>
      ...
      {active.length > 1 && (
        <div className="chips" style={{ marginBottom: "1rem" }}>
          {active.map((m) => <Link key={m.tenant_id} href={`/settings?tenant=${m.tenant_id}`} className={m.tenant_id === tenant ? "active" : ""}>{m.tenant_id}</Link>)}
        </div>
      )}
      <Settings
        tenant={tenant}
