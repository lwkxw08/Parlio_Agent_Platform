...
def test_cost_rates_per_provider_and_plan_defaults() -> None:
    ...
    assert (plans["starter"].monthly_pence, plans["starter"].included_minutes) == (7900, 500)
    assert (plans["starter"].included_sms, plans["starter"].sms_overage_pence) == (150, 6)
    assert (plans["growth"].monthly_pence, plans["growth"].included_minutes) == (23900, 2000)
    assert (plans["growth"].included_sms, plans["growth"].overage_pence_per_minute) == (600, 10)
    assert (plans["scale"].monthly_pence, plans["scale"].included_minutes) == (55900, 5000)
