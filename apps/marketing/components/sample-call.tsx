...
import samples from "@/lib/sample-calls.json";

type Cue = { who: "ai" | "caller"; text: string; start: number; end: number };
type Sample = { id: string; title: string; business: string; ai: string; src: string; duration: number; cues: Cue[] };
...
/** Pre-recorded, fully synthetic calls (one per industry) with a transcript that follows the audio. */
export function SampleCall() {
  ...
  return (
    <div className="sample-call">
      <div className="sample-tabs" role="tablist" aria-label="Sample calls">
        {SAMPLES.map((s) => (
          <button key={s.id} type="button" role="tab" aria-selected={s.id === sample.id} className={`sample-tab ${s.id === sample.id ? "on" : ""}`} onClick={() => setSampleId(s.id)}>
            {s.title}
          </button>
        ))}
      </div>
      <div className="sample-head">
        ...
        <div className="meta">
          <b>Sample call · {sample.title.toLowerCase()}</b>
          <span className="muted small">{sample.business} (fictional) · {sample.ai} answers · {mmss(duration)}</span>
        </div>
      ...
      <div className="sample-lines" ref={list}>
        {cues.map((c, i) => (
          <div key={`${sample.id}-${i}`} className={`line ${c.who} ${i === reached ? "now" : ""} ${i < reached ? "done" : ""}`} onClick={() => { const a = audio.current; if (a) { a.currentTime = c.start / 1000; setPos(c.start); if (a.paused) void a.play(); } }}>
            <span className="who">{c.who === "ai" ? sample.ai : "Caller"}</span>
            <span>{c.text}</span>
          </div>
        ))}
      </div>
      <p className="muted small sample-note">Generated for this demo with the same voice engine tenants use — a different voice for each business. Every name, number and postcode is fictional.</p>
    </div>
  );
}
